...
static int eal_parse_args(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argvopt, eal_short_options,
        ...
        uk_pr_debug("LCORE_ROLE 0:%d\n",rte_eal_get_configuration()->lcore_role[0]);
        uk_pr_debug("LCORE_ROLE 1:%d\n",rte_eal_get_configuration()->lcore_role[0]);
        uk_pr_debug("enabled 0:%d\n",rte_lcore_is_enabled(0));
        uk_pr_debug("enabled 1:%d\n",rte_lcore_is_enabled(1));
        uk_pr_debug("2.master:%d\n",rte_eal_get_configuration()->master_lcore);
	if (eal_adjust_config(&internal_config) != 0) {
		// rc = -1;
        // uk_pr_debug("6\n");
        // uk_pr_debug("3.master:%d\n",rte_eal_get_configuration()->master_lcore);
		// goto out;
        }
...
{
        uk_pr_debug("sync at core%d",rte_lcore_id());
	return 0;
}
...
int rte_eal_init(int argc, char **argv)
{
	...
	rte_config_init();
        uk_pr_debug("start hugepage info init\n");
	/**
	...
	if (rc < 0) {
        ...
        uk_pr_debug("start memzone init\n");
	/**
	...
	RTE_LCORE_FOREACH_SLAVE(i) {
        ...
        uk_pr_debug("lcpu run\n");
	r = ukplat_lcpu_run(idx, &num, &loop_func, 0);
        uk_pr_debug("lcpurun state:%d\n",r);
        uk_pr_debug("remote launch loop\n");
	r = rte_eal_mp_remote_launch(sync_func, NULL, SKIP_MASTER);
        uk_pr_debug("rmlaunch state:%d\n",r);
        uk_pr_debug("wait lcore\n");
	rte_eal_mp_wait_lcore();

#endif /* CONFIG_HAVE_SMP */
        uk_pr_debug("netdevice init\n");
	r = eal_uknetdev_init();
	if (r < 0) {
